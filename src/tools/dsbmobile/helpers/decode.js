    ...
    if (nTranscrType < 8) {
        ...
        conversionSwitch: switch (nTranscrType) {
            ...
            case 1:
                ...
                for (
                    let nInptIdx = nStartIdx, nOutptIdx = 0;
                    nOutptIdx < nOutptLen;
                    nInptIdx++
                ) {
...
StringView.prototype.subview = function (
    ...
    const nThisLen = bVariableLen ? this.makeIndex() : nRawLen,
...
StringView.prototype.valueOf = StringView.prototype.toString = function () {
    ...
    for (
        let nChr, nLen = this.rawData.length, nIdx = 0;
        nIdx < nLen;
        nIdx += fGetIncr(nChr)
    ) {
